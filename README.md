...
1. Search for `'revera'` (inside single quotations) to capture the text domain.
2. Search for `revera_` to capture all the function names.
3. Search for <code>&nbsp;_tk</code> (with a space before it) to capture DocBlocks.
4. Search for `revera-` to capture prefixed handles.

OR

* Search for: `'revera'` and replace with: `'megatherium'`
* Search for: `revera_` and replace with: `megatherium_`
